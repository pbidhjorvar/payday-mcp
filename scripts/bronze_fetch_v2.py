...
def fetch_resource(resource):
    ...
    tool_mapping = {
        ...
        # Note: transactions are handled via account statement
        "transactions": "payday_get_account_statement", 
    }
    ...
    # Special handling for transactions (account statement)
    if resource == "transactions":
        # For transactions, we need date range - use last 90 days as default
        ...
        print(f"[INFO] Fetching transactions from {start_date} to {end_date}")
        result = call_mcp_tool(tool_name, args)
    else:
...
def main():
    print("[INFO] Using Payday-MCP tools for data fetching")
