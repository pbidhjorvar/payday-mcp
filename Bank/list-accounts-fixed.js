...
async function getAccountMeta(client, seta, parts) {
  ...
  return {
    ok: true,
    currency: cm && cm[1] ? cm[1].trim() : undefined,
    status: smeta && smeta[1] ? smeta[1].trim() : undefined,
  };
}
...
async function main() {
  ...
  for (const a of accounts) {
    const parts = parseParts(a.number);
    if (!parts) { out.push({ ...a, ok: false, error: 'Invalid account number format' }); continue; }
    const exists = await accountExists(client, seta, parts, kennitala);
    if (!exists.ok) { out.push({ ...a, ok: false, error: exists.error }); continue; }
    const meta = await getAccountMeta(client, seta, parts);
    if (meta.ok) out.push({ ...a, ok: true, currency: meta.currency, status: meta.status });
    else out.push({ ...a, ok: true, currency: null, status: null, note: meta.error });
  }
